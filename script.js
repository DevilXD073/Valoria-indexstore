...
/* ---------- STORE CONFIG ---------- */
/* EDIT: server info, discord link, and payment URL here */
const STORE_CONFIG = {
  ...
  currency: "₹",
  // Leave empty until a real payment provider (Razorpay / UPI / Stripe / etc.)
  // is connected. When empty, checkout shows a "not connected yet" notice.
  paymentUrl: ""
};
  ...
  grid.innerHTML = ranks.map(r => {
    ...
    return `
      <div class="rank-card ${featured ? "is-featured" : ""}">
        ...
        <h3 class="rank-name">${r.icon} ${r.name}</h3>
...
function renderCheckoutSummary() {
  ...
  if (!STORE_CONFIG.paymentUrl) {
    ...
    notice.textContent = "Payment gateway isn't connected yet.";
  } else {
...
function bindCheckout() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (!STORE_CONFIG.paymentUrl) {
      showToast("Payment gateway isn't connected yet.");
      return;
    }
