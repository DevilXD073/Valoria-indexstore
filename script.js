...
function renderPaymentStep() {
  ...
  const discount = cartDiscount();
  document.getElementById("paySubtotalRow").style.display = discount > 0 ? "flex" : "none";
  document.getElementById("payDiscountRow").style.display = discount > 0 ? "flex" : "none";
  if (discount > 0) {
    document.getElementById("paySubtotal").textContent = formatPrice(cartSubtotal());
    document.getElementById("payCouponCode").textContent = appliedCoupon;
    document.getElementById("payDiscountAmount").textContent = `-${formatPrice(discount)}`;
  }
