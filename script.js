...
const PRODUCTS = [
  ...
  },
  {
    ...
    features: ["Basic rank perks", "Starter kit", "2x crate keys"]
  },
  {
    id: "pvp-bundle",
    name: "PvP Bundle",
    ...
    features: ["PvP kit", "Combat perks", "3x crate keys"]
  },
  {
    id: "energy-pack",
    name: "Energy Pack",
    ...
    category: "perks",
    icon: "🔋",
    description: "A quick boost of extras to keep you moving.",
    features: ["XP boost", "Extra sethome", "Faster cooldowns"]
  },
  {
    id: "crate-key",
    name: "Crate Key",
    ...
    category: "keys",
    icon: "🗝️",
    description: "Open a crate for a chance at rare rewards.",
    features: ["1x crate key", "Random reward tier", "Stackable"]
  }
];
...
function showCheckoutStep(step) {
  document.getElementById("checkoutStepForm").style.display = step === "form" ? "block" : "none";
  document.getElementById("checkoutStepPayment").style.display = step === "payment" ? "block" : "none";
}
...
function renderPaymentStep() {
  ...
  document.getElementById("payQrImage").src = STORE_CONFIG.qrImagePath;
  document.getElementById("payUpiId").textContent = STORE_CONFIG.upiId;
  ...
  document.getElementById("payOrderTotal").textContent = formatPrice(cartTotal());
}
...
function bindCheckout() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (mcVal.length < 3 || mcVal.length > 16 || !/^[A-Za-z0-9_]+$/.test(mcVal)) {
      mcInput.classList.add("is-invalid");
      mcError.textContent = "Enter a valid Minecraft username (3-16 characters, letters/numbers/underscore).";
      valid = false;
    }
  ...
  document.getElementById("payGoDiscordBtn").addEventListener("click", () => {
    clearCart();
    hideModal("checkoutModal", "checkoutOverlay");
    showToast("Order noted — send your screenshots to staff on Discord");
  });
}
